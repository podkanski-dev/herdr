...
### Added
- Recognize and resume custom per-account Claude commands via the new `[agents]` config, and install the hook into multiple account config dirs with `config_dirs` or `--config-dir`.
- Added per-workspace accent colors, set from the workspace context menu "Set color" entry and persisted via the `[workspace_colors]` config.
