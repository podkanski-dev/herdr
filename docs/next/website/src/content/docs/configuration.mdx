...
Give a workspace its own accent color and Herdr paints a one-column accent bar down the left edge of its card in the sidebar.

There are two layers:

**Per workspace (set in the app).** Right-click a workspace and choose **Set color** to open the color picker. Pick a curated theme swatch, type a custom hex value into the `hex:` field, or choose **none** to clear it. Move between choices with the arrow keys (or `Tab`), press `Enter` to save, and `Esc` to cancel; you can also click a swatch, the hex field, or the **save** / **cancel** buttons. This color belongs to that specific workspace, persists with the session (per named session), and is independent of the working directory — two workspaces in the same directory can have different colors, and the color stays put if the working directory changes.

**Per directory (set in config).** You can also map a working directory to a default color in `config.toml`. Any workspace whose working directory matches picks up that color automatically, unless it has its own per-workspace color set in the app:

```toml
[workspace_colors]
"/home/you/code/herdr" = "blue"
"/home/you/code/website" = "#f38ba8"
```

A workspace's effective color is its per-workspace color if set, otherwise the directory default for its working directory, otherwise none. New workspaces start with no per-workspace color. Curated swatches follow your active theme palette; custom values accept the same formats as theme colors (hex, named colors, and `rgb(r,g,b)`). Invalid or unparseable values are ignored.
...
If you run multiple Claude accounts through separate executables — for example, wrapper scripts that set a different `CLAUDE_CONFIG_DIR` before invoking the real agent — you can tell Herdr to recognize those commands as the built-in agent and resume panes with the same command that launched them.
...
The `[agents.<id>]` table is keyed by a built-in agent id such as `claude` or `codex`. An unknown id is ignored with a startup warning.

`commands` lists extra executable names Herdr should treat as that agent. When a pane's foreground process matches one of these names, Herdr applies the agent's screen manifest for state detection. On native session restore, Herdr relaunches that same command instead of the default agent executable. Each name must be a real executable on your `PATH`. A wrapper script or symlink works; a plain shell alias does not, because Herdr both detects and re-launches a real process.

`config_dirs` lists additional account config directories the integration installer should target. Each entry is a path to a directory that holds its own `settings.json` — typically the directory a wrapper sets as `CLAUDE_CONFIG_DIR`. `~` is expanded. See [Integrations](/docs/integrations/#claude-code) for how to install the hook into multiple config dirs.
