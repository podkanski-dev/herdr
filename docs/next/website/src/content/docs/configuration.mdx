...
Give a workspace its own accent color and Herdr paints a one-column accent bar down the left edge of its card in the sidebar, so you can tell workspaces apart at a glance.

Right-click a workspace and choose **Set color** to open the color picker. Pick one of the curated theme swatches, type a custom hex value into the `hex:` field, or choose **none** to clear the color. Move between choices with the arrow keys (or `Tab`), press `Enter` to save, and `Esc` to cancel. You can also click a swatch, the hex field, or the **save** / **cancel** buttons directly.

Curated swatches follow your active theme palette, so they stay readable when you switch themes. Custom values accept the same formats as theme colors: hex, named colors, and `rgb(r,g,b)`.

Colors are saved to your config under `[workspace_colors]`, keyed by each workspace's working directory:
...
You normally manage this table through the picker rather than editing it by hand. Invalid or unparseable values are ignored on load.
