...
/// Parse a color string into a ratatui Color, returning `None` for
/// unrecognized input. Use this when input must be validated (e.g. the
...
/// Unknown input warns and defaults to cyan.
...
"##;
    ...
    #[test]
    fn try_parse_color_accepts_known_values() {
        use ratatui::style::Color;
        assert_eq!(try_parse_color("#89b4fa"), Some(Color::Rgb(137, 180, 250)));
        assert_eq!(try_parse_color("blue"), Some(Color::Blue));
        assert_eq!(try_parse_color("rgb(1, 2, 3)"), Some(Color::Rgb(1, 2, 3)));
        assert_eq!(try_parse_color("none"), Some(Color::Reset));
    }

    #[test]
    fn try_parse_color_rejects_garbage() {
        assert_eq!(try_parse_color("not-a-color"), None);
        assert_eq!(try_parse_color("#12"), None);
        assert_eq!(try_parse_color("#zzzzzz"), None);
        assert_eq!(try_parse_color(""), None);
    }

    #[test]
    fn parse_color_still_defaults_unknown_to_cyan() {
        use ratatui::style::Color;
        assert_eq!(parse_color("totally-unknown"), Color::Cyan);
    }
}
