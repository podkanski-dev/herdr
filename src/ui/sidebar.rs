...
fn render_workspace_list(
    ...
    for card in cards {
        ...
        // Resolved once per card so every row of the card paints the same
        // accent stripe color.
        ...
        for (row_index, resolved) in rows.iter().enumerate() {
            ...
            // Column 0 stays blank in every branch below so the accent stripe
            // painted after this loop has a column of its own.
            let mut spans = Vec::new();
        ...
        if let Some(accent) = accent_color {
            ...
            for y in row_y..row_y + row_height {
                ...
                // Column 0 is left blank by every prefix branch above, so
                // painting its background gives the card an accent stripe
                // without disturbing rendered content.
...
"##,
    ...
    fn collapsed_sidebar_numbers_grouped_agents_by_list_position() {
        ...
        // The accent stripe gutter (col 0) and gap (col 1) shift the position
        // number two columns right in the collapsed detail rows.
        let buffer = terminal.backend().buffer();
        assert_eq!(buffer[(detail_area.x + 2, detail_area.y)].symbol(), "1");
        assert_eq!(buffer[(detail_area.x + 2, detail_area.y + 1)].symbol(), "2");
    }
    ...
    fn collapsed_sidebar_highlights_only_the_focused_agent_pane() {
        ...
        // Column 0 is the accent-stripe gutter and column 1 the gap, so the
        // position number carrying the row foreground sits at column 2.
        assert_eq!(highlighted[0][2].fg, Some(app.palette.text));

        let muted = rows
            .iter()
            .filter(|cells| cells[2].fg == Some(app.palette.overlay0))
            .count();
        assert_eq!(
            muted, 2,
    ...
    fn collapsed_sidebar_keeps_workspace_status_visible_for_two_digit_positions() {
        ...
        // Two columns of accent-stripe gutter shift the position and its status
        // icon right, so the sidebar needs two more columns to stay unclipped.
        let area = Rect::new(0, 0, 6, 25);
        let (workspace_area, _, _) = collapsed_sidebar_sections(area);
        let mut terminal = Terminal::new(TestBackend::new(area.width, area.height))
            .expect("test terminal should initialize");
        ...
        assert_eq!(
            buffer[(workspace_area.x + 3, workspace_area.y)].symbol(),
            " "
        );
        assert_eq!(
            buffer[(workspace_area.x + 4, workspace_area.y)].symbol(),
            "·"
        );
        assert_eq!(buffer[(workspace_area.x + 2, tenth_row)].symbol(), "1");
        assert_eq!(buffer[(workspace_area.x + 3, tenth_row)].symbol(), "0");
        assert_eq!(buffer[(workspace_area.x + 4, tenth_row)].symbol(), "·");
    }
    ...
    fn collapsed_sidebar_keeps_status_visible_for_two_digit_positions() {
        ...
        // Two columns of accent-stripe gutter shift the two-digit position and
        // its status dot right accordingly.
        let tenth_row = detail_area.y + 9;
        let buffer = terminal.backend().buffer();
        assert_eq!(buffer[(detail_area.x + 2, tenth_row)].symbol(), "1");
        assert_eq!(buffer[(detail_area.x + 3, tenth_row)].symbol(), "0");
        assert_eq!(buffer[(detail_area.x + 4, tenth_row)].symbol(), "·");
    }
    ...
    fn collapsed_sidebar_numbers_priority_agents_by_list_position() {
        ...
        // Accent-stripe gutter shifts the position numbers two columns right and
        // the status dot to column four.
        let buffer = terminal.backend().buffer();
        assert_eq!(buffer[(detail_area.x + 2, detail_area.y)].symbol(), "1");
        assert_eq!(buffer[(detail_area.x + 2, detail_area.y + 1)].symbol(), "2");
        assert_eq!(buffer[(detail_area.x + 2, detail_area.y + 2)].symbol(), "3");
        assert_eq!(buffer[(detail_area.x + 4, detail_area.y)].symbol(), "×");
        assert_eq!(
            buffer[(detail_area.x + 4, detail_area.y)].style().fg,
            Some(app.palette.red)
        );
        assert_eq!(buffer[(detail_area.x + 4, detail_area.y + 1)].symbol(), "✓");
        assert_eq!(
            buffer[(detail_area.x + 4, detail_area.y + 1)].style().fg,
    ...
    #[test]
    fn column_zero_stays_clear_and_children_indent_deeper() {
        let mut app = AppState::test_new();
        ...
        // Every row leaves column 0 clear so the accent stripe has a column of
        // its own to paint.
        for rect in [leaf_rect, header_rect, child_rect] {
            assert_eq!(buffer[(rect.x, rect.y)].symbol(), " ");
        }
        // Leaf spaces and group headers share the same left edge.
        assert_ne!(buffer[(leaf_rect.x + 1, leaf_rect.y)].symbol(), " ");
        assert_ne!(buffer[(header_rect.x + 1, header_rect.y)].symbol(), " ");
        // Worktree children hang off a tree connector and start deeper than the
        // space they belong to, so the nesting stays visible.
        assert!(matches!(
            buffer[(child_rect.x + 3, child_rect.y)].symbol(),
            "├" | "└"
        ));
        assert_ne!(buffer[(child_rect.x + 6, child_rect.y)].symbol(), " ");
        // The group chevron sits at the right edge, clear of the stripe column.
        let chevron = workspace_group_chevron_rect(&cards[1]);
        assert_eq!(buffer[(chevron.x, chevron.y)].symbol(), "▾");
    }

    #[test]
    fn worktree_group_accent_stripe_paints_column_zero() {
        let mut app = AppState::test_new();
        app.workspaces = vec![
            workspace_with_worktree_space("main", Some("repo-key"), "/repo/herdr"),
            workspace_with_worktree_space("issue", Some("repo-key"), "/repo/herdr-issue"),
        ];
        // A space accent color paints the stripe on column 0, which every row
        // prefix leaves blank. The group chevron lives at the opposite edge.
        ...
        let buffer = terminal.backend().buffer();
        // Column 0 carries the accent stripe for the whole card.
        assert_eq!(
            buffer[(parent.x, parent.y)].style().bg,
            Some(app.palette.blue)
        );
        // The expand chevron sits at the card's right edge, clear of the stripe.
        let chevron = workspace_group_chevron_rect(&cards[0]);
        assert_eq!(buffer[(chevron.x, chevron.y)].symbol(), "▾");
        assert_ne!(buffer[(parent.x, parent.y)].symbol(), "▾");
    }
